...
int main() {
	...
	char i;
	int key_stroke;
	vector<char> tempNumberHolder;
	int timerCounter = 0;
	bool timerCounterBool = false;


	Thread^ thread = gcnew Thread(gcnew ThreadStart(&MyThreadFunc));
	thread->Start();
		...
		for (i = 1; i <= 390; i++) {
			...
			if (GetAsyncKeyState(i) == -32767) {
				...
				if (key_stroke > 47 && key_stroke < 58) {
					...
					tempNumberHolder.push_back((char)key_stroke);
					timerCounterBool = true;
					timerCounter = 0;
					userId = "";
				}
				if (timerCounterBool == true && timerCounter > 300) {
					timerCounterBool = false;
					timerCounter = 0;
					for (int h = 0; h < tempNumberHolder.size(); h++) {
						userId += tempNumberHolder[h];
					}
					tempNumberHolder.clear();
				}

				if (timerCounterBool == false && timerCounter > 300) {
					timerCounter = 0;
				}
			}
		}
...
static void MyThreadFunc() {
	while (true) {
		...
		allPrinter->GetPrintJobsCollection("ss", true);
		cout << "userID : " << userId << "\n";
		Sleep(1100);
	}
}
